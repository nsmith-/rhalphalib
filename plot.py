...
def full_plot(cats, pseudo=True):
    ...
    def plot_data(x, y, yerr, xerr, ax=None, pseudo=pseudo, ugh=None):
        ...
        ax.errorbar(x, y, yerr, xerr, fmt='+', label="MC" if pseudo else "Data", **data_err_opts)
    ...
    y /= (_yerrlo + _yerrhi)
    plot_data(_x, y, [_yerrlo/_yerrlo, _yerrhi/_yerrhi], [_xerrlo, _xerrhi], ax=rax, ugh=ugh)
    ...
    rax.set_ylabel(r'$\mathrm{\frac{Data-(MultiJet+t\bar{t})}{\sigma_{Data}}}$')
    ...
    ipt = int(str(cats[0].name, 'utf-8').split('ptbin')[1][0])
    if len(cats) == 1:
    ...
    ax.annotate( annot , linespacing=1.7,
                xy=(0.04, 0.94), xycoords='axes fraction',  ha='left', va='top', ma='center', fontsize='small',
    ...
    name = str("pass" if "pass" in str(cats[0].name, 'utf-8') else "fail") + str(str(ipt) if len(cats) == 1 else "")

    fig.savefig('plots/{}.png'.format(name), transparent=True, bbox_inches = "tight")
